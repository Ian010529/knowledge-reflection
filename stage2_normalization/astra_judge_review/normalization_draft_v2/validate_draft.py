...
assert len(am)==4646 and len(cm)==3746 and len(rels)==2655
seen=[]
for c in concepts:
...
queue=read(OUT/'astra_spotcheck_results.csv')
assert len(queue)==len({r['group_index'] for r in queue})==50
assert all(r['status']=='completed' and r['required_human_review']=='False' for r in queue)
...
result={'status':'passed','raw_node_rows_preserved':4646,'raw_relation_rows_preserved':2655,
    ...
    'astra_spotchecks_completed':50,'model_context_flags_reviewed':len(context),'human_review_required':False,
