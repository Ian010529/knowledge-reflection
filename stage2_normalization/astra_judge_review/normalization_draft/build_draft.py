...
def build():
    inputs = {
        ...
        **{f'judge/{n}': JUDGE/n for n in ['node_assignments_draft.csv','pair_judgments.csv','group_judgments.jsonl','group_review.csv','hierarchy_flag_review.csv','within_group_non_equivalent_review.csv','original_uncertain_disposition.csv','duplicate_conflict_judgment.json','input_manifest.json','progress.json','METHOD_CHANGE.md']},
    ...
    for key,mids in sorted(partitions.items()):
        ...
        row={'concept_id':cid,'domain':domain,'display_label':label,
             ...
             'source_roles':js(roles),'canonical_role':'','role_status':'deferred' if deferred_here else ('mixed_source_roles_need_check' if len(roles)>1 else 'source_role_preserved_not_newly_validated'),
             ...
             'concept_status':'draft_only','human_acceptance_status':'pending','run_id':run_id}
    ...
    # Reproducible, risk-directed human worksheet; not the frozen extraction audit.
    grows={int(r['group_index']):r for r in group_rows}
    hierarchy=read(JUDGE/'hierarchy_flag_review.csv')
    reversed_counts=collections.Counter(int(r['group_index']) for r in hierarchy if r['judge_equivalence_decision']=='accept')
    mixed_groups=sorted({int(r['judge_partition_id'].split(':')[1][1:]) for r in mixed})
    selected={i:'mixed_source_roles' for i in mixed_groups}
    def pick(indices, count, why):
        n=0
        for i in indices:
            if i in selected:continue
            selected[i]=why;n+=1
            if n==count:break
    pick(sorted(grows,key=lambda i:(-int(grows[i]['original_members']),i)),10,'large_group')
    pick(sorted(reversed_counts,key=lambda i:(-reversed_counts[i],i)),8,'hierarchy_warning_overruled')
    pick([i for i in grows if grows[i]['judge_decision']=='split'],6,'split_definition_boundary')
    pick([i for i in grows if grows[i]['judge_decision']=='uncertain'],4,'uncertainty_boundary')
    pick(sorted((i for i in grows if grows[i]['judge_decision']=='accept'),key=lambda i:hashlib.sha256(f'normalization-spotcheck-v1:{i}'.encode()).hexdigest()),50-len(selected),'accepted_group_reference')
    assert len(selected)==50
    worksheet=[]; audit_nodes=[]
    for i,reason in selected.items():
        mids=sorted(m for m in rm if int(rm[m]['group_index'])==i)
        worksheet.append({'audit_id':f'normcheck:g{i:03d}','group_index':i,'selection_reason':reason,
            'original_members':len(mids),'judge_decision':grows[i]['judge_decision'],
            'draft_concept_ids':js(sorted({canonical[m] for m in mids})),
            'raw_mention_ids':js(mids),'judge_reason':grows[i]['judge_reason'],
            'review_prompt':'逐成员核对共同定义、限定词、机制/属性区别、材料/器件层次及证据；接受分区是否过宽，拒绝或待定是否过严。',
            'human_decision':'','human_reason':'','reviewer':'','reviewed_at':'','status':'pending_human_review'})
        for mid in mids:
            audit_nodes.append(dict(nm[mid],audit_id=f'normcheck:g{i:03d}',draft_concept_id=canonical[mid],
    ...
    write('normalization_spotcheck_queue.csv',worksheet)
    write('normalization_spotcheck_evidence.csv',audit_nodes)
    save_json('spotcheck_design.json',{'count':50,'selection':'risk-directed, deterministic; not probability sampling',
        'strata_counts':dict(collections.Counter(selected.values())),
        'human_reviews_completed':0,'accuracy_estimate_available':False,
    ...
    summary={'run_id':run_id,'status':'draft_generated_not_applied','raw_nodes':len(nodes),'reviewed_nodes':len(rm),
        ...
        'mixed_role_concepts':len(mixed),'normalization_human_spotcheck_groups':len(worksheet),
        'human_spotchecks_completed':0,'source_mutations':0,'formal_graph_built':False,
