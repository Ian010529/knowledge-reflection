...
DATA = Path("/Users/ryan/Documents/iTE&TG/final_concept_layer")
OUT = ROOT / "work" / "final_concept_ite_prediction"
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    papers = papers[papers["source_membership"].str.contains("iTE", na=False)].copy()
...
def make_candidates(
    ...
    state = graph_state(mapping, cutoff, vocab)
    future_sets = paper_concept_sets(mapping, cutoff + 1, horizon_end)
    future_edges = edge_set_from_papers(future_sets)
    rows = []
    for u, v in combinations(state["eligible"], 2):
        ...
        features["future_edge_label"] = int(pair in future_edges)
...
def plot_performance(scored: pd.DataFrame, metrics: pd.DataFrame) -> None:
    ...
    fig.suptitle(
        "iTE temporal backtest: train through 2022, test on first co-occurrence in 2023-2026",
        x=0.01,
        ha="left",
        fontsize=9,
    )
    save_pub(fig, OUT / "ite_final_concept_roc_pr")
    plt.close(fig)
...
def plot_confusion_matrix(scored: pd.DataFrame, classification: dict) -> None:
    ...
    save_pub(fig, OUT / "ite_final_concept_confusion_matrix")
    plt.close(fig)
...
def plot_prediction_matrix(scored: pd.DataFrame, top_pairs: int = 120) -> None:
    ...
    fig.suptitle(
        "Predicted versus observed links among top-ranked concepts",
        x=0.01,
        ha="left",
        fontsize=9,
    )
    save_pub(fig, OUT / "ite_final_concept_prediction_matrix")
    plt.close(fig)
    ...
    predicted_frame.to_csv(OUT / "ite_prediction_matrix_top_concepts.csv")
    observed_frame.to_csv(OUT / "ite_observed_matrix_top_concepts.csv")
...
def main() -> None:
    ...
    train_windows = []
    window_summary = []
    for cutoff in range(2012, 2019):
        frame = make_candidates(mapping, vocab, embeddings, cutoff, cutoff + 3)
        if frame.empty:
            continue
    ...
    rolling_validations = {
        2017: make_candidates(mapping, vocab, embeddings, 2017, 2020),
        2018: make_candidates(mapping, vocab, embeddings, 2018, 2021),
        2019: make_candidates(mapping, vocab, embeddings, 2019, 2022),
    }
    validation = rolling_validations[2019]
    test = make_candidates(mapping, vocab, embeddings, 2022, 2026)
    ...
    train.to_csv(OUT / "ite_train_samples_2012_2018.csv", index=False)
    validation.to_csv(OUT / "ite_validation_candidates_2019_to_2022.csv", index=False)
    test.to_csv(OUT / "ite_test_candidates_2022_to_2026.csv", index=False)
    scored.to_csv(OUT / "ite_scored_candidates_2022_to_2026.csv", index=False)
    scored.head(100).to_csv(OUT / "ite_top100_predictions.csv", index=False)
    metrics.to_csv(OUT / "ite_backtest_metrics.csv", index=False)
    tuning.to_csv(OUT / "ite_hyperparameter_tuning.csv", index=False)
    pd.DataFrame(window_summary).to_csv(OUT / "ite_training_window_summary.csv", index=False)
    pd.DataFrame([classification]).to_csv(
        OUT / "ite_classification_metrics.csv", index=False
    )
    ...
    feature_importance.to_csv(OUT / "ite_model_feature_dictionary.csv", index=False)
    ...
    summary = {
        "input_unique_iTE_papers": int(len(papers)),
        "input_prediction_core_concepts": int(len(vocab)),
        "concepts_seen_at_2022": int(
            mapping[mapping["year"] <= 2022]["concept_id"].nunique()
        ),
    ...
    (OUT / "ite_prediction_summary.json").write_text(
