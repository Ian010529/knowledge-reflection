...
OUT = ROOT / "minimal_clean_predictions" / "tg_to_tg"
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    papers = papers[papers["source_membership"].str.contains("TG", na=False)].copy()
...
def plot_performance(scored: pd.DataFrame, metrics: pd.DataFrame) -> None:
    ...
    fig.suptitle(
        "TG temporal backtest: train through 2022, test on first co-occurrence in 2023-2026",
        x=0.01,
        ha="left",
        fontsize=9,
    )
    save_pub(fig, OUT / "tg_self_roc_pr")
    plt.close(fig)
...
def plot_confusion_matrix(scored: pd.DataFrame, classification: dict) -> None:
    ...
    save_pub(fig, OUT / "tg_self_confusion_matrix")
    plt.close(fig)
...
def plot_prediction_matrix(scored: pd.DataFrame, top_pairs: int = 120) -> None:
    ...
    fig.suptitle(
        "Predicted versus observed links in TG",
        x=0.01,
        ha="left",
        fontsize=9,
    )
    save_pub(fig, OUT / "tg_self_prediction_matrix")
    plt.close(fig)
    ...
    predicted_frame.to_csv(OUT / "tg_self_prediction_matrix_top_concepts.csv")
    observed_frame.to_csv(OUT / "tg_self_observed_matrix_top_concepts.csv")
...
def main() -> None:
    ...
    train.to_csv(OUT / "tg_self_train_samples_2012_2018.csv", index=False)
    validation.to_csv(OUT / "tg_self_validation_candidates_2019_to_2022.csv", index=False)
    test.to_csv(OUT / "tg_self_test_candidates_2022_to_2026.csv", index=False)
    scored.to_csv(OUT / "tg_self_scored_candidates_2022_to_2026.csv", index=False)
    scored.head(100).to_csv(OUT / "tg_self_top100_predictions.csv", index=False)
    metrics.to_csv(OUT / "tg_self_backtest_metrics.csv", index=False)
    tuning.to_csv(OUT / "tg_self_hyperparameter_tuning.csv", index=False)
    pd.DataFrame(window_summary).to_csv(OUT / "tg_self_training_window_summary.csv", index=False)
    pd.DataFrame([classification]).to_csv(
        OUT / "tg_self_classification_metrics.csv", index=False
    )
    ...
    feature_importance.to_csv(OUT / "tg_self_model_feature_dictionary.csv", index=False)
    ...
    summary = {
        "input_unique_TG_papers": int(len(papers)),
    ...
    (OUT / "tg_self_prediction_summary.json").write_text(
