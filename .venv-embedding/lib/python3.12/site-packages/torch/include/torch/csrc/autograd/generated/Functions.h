...
#ifdef _WIN32
struct LinalgMatrixSqrthBackward0 : public TraceableFunction {
  TORCH_API LinalgMatrixSqrthBackward0() = default;
#else
struct TORCH_API LinalgMatrixSqrthBackward0 : public TraceableFunction {
#endif
  using TraceableFunction::TraceableFunction;
  variable_list apply(variable_list&& grads) override;
  std::string name() const override { return "LinalgMatrixSqrthBackward0"; }
  void release_variables() override {
    std::lock_guard<std::mutex> lock(mutex_);
    self_.reset_data();
  }
...
#ifdef _WIN32
struct LinalgPolarBackward0 : public TraceableFunction {
  TORCH_API LinalgPolarBackward0() = default;
#else
struct TORCH_API LinalgPolarBackward0 : public TraceableFunction {
#endif
  using TraceableFunction::TraceableFunction;
  variable_list apply(variable_list&& grads) override;
  std::string name() const override { return "LinalgPolarBackward0"; }
  void release_variables() override {
    std::lock_guard<std::mutex> lock(mutex_);
    A_.reset_data();
    H_.reset_data();
