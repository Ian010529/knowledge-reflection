...
**当前主线：证据化 iTE/TG 双图谱与受控跨领域 match。** 后续先读 [抽取与执行约定](stage2_normalization/CROSS_DOMAIN_MATCH_EXTRACTION.md)，再读 [最新状态](stage2_normalization/astra_judge_review/CURRENT_STATUS.md)。约定已综合研究计划，精简抽取工程，保留域内 landscape、四组匹配对照及案例核查。
...
- **抽检和一次定向纠错已完成**：[报告](stage2_normalization/full_audit_v1/REPORT.md)。当前修订数据为 21,115 条关系，其中 20,984 条接受、131 条待定。原始全量加权严格精确率 iTE 80.0%、TG 82.1%；没有重新估计修订后总体质量，不能宣称已达到 90%。
...
详细执行状态：[最新状态](stage2_normalization/astra_judge_review/CURRENT_STATUS.md)。历史 12/40/60 篇试验及旧归一化记录继续保留，旧逐对入口保持停用。旧文档中的“尚未全量/尚未建图/尚未匹配”仅是当时状态。
